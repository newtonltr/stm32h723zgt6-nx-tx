...
/**************************************************************************/
/*
  以太网驱动程序，协议栈对以太网驱动发起请求时调用此函数，
  参数包含了请求类型等信息，驱动程序根据请求类型进行处理。
*/
...
static VOID  _nx_driver_enable(NX_IP_DRIVER *driver_req_ptr)
{
  ...
  if (nx_eth_phy_init() != DP83848_STATUS_OK)
  {
    driver_req_ptr -> nx_ip_driver_status =  NX_DRIVER_ERROR;
    return;
  }
...
#if 0
	#include "stdio.h"
	int buf_index = packet_ptr->nx_packet_length;
	printf("hard driver:");
	while(buf_index--){
		printf("%x ", Txbuffer[0].buffer[buf_index]);
	}
	printf("\r\n");

	buf_index = TxPacketCfg.Length;
	printf("st hal driver:");
	while(buf_index--){
		printf("%x ", TxPacketCfg.TxBuffer->buffer[buf_index]);
	}
	printf("\r\n");
#endif
...
void HAL_ETH_RxCpltCallback(ETH_HandleTypeDef *heth)
{
  ...
  //if(nx_driver_information.nx_driver_information_ip_ptr == NULL){
	//  return;
  //}
